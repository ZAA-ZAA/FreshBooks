...
export default function InvoicesList() {
    ...
    const handleRecordPayment = (inv: any, e: React.MouseEvent) => {
        e.stopPropagation();
        // Redirect to payments with invoice context to pre-fill the quick add form
        navigate('/payments', { state: { quickAdd: true, invoiceNumber: inv.number, client: inv.client, amount: inv.amount, invoiceId: inv.id } });
    };

    const overdue = invoices.filter(i => i.status === 'Overdue').reduce((acc, curr) => acc + (parseFloat(curr.amount) || 0), 0);
    const outstanding = invoices.filter(i => i.status !== 'Paid').reduce((acc, curr) => acc + (parseFloat(curr.amount) || 0), 0);
    const draft = invoices.filter(i => i.status === 'Draft').reduce((acc, curr) => acc + (parseFloat(curr.amount) || 0), 0);
                        ...
                        {filteredInvoices.map(inv => (
                            <tr key={inv.id} className={`hover:bg-[#f0f9ff]/50 cursor-pointer group transition-colors ${selectedIds.includes(inv.id) ? 'bg-[#f0f9ff]' : ''}`} onClick={() => navigate(`/invoices/${inv.id}`)}>
                                ...
                                <td className="p-4">
                                    ...
                                    <div className="text-[10px] text-gray-300">{inv.dateDue || '—'}</div>
                                </td>
                                <td className="p-4 text-right">
                                    <div className="font-bold text-[#2d3a4b] text-[13px]">₱{inv.amount.toLocaleString()}</div>
                                    <div className={`text-[10px] font-black uppercase px-2 py-0.5 rounded border inline-block mt-1 ${getStatusStyle(inv.status)}`}>{inv.status}</div>
                                </td>
